...
class blogController extends ZenController
{
    ...
    function index($request_data = array())
    {
         ...
         */
        $link_like = '<a href="' . $blog_data['full_url'] . '?_like_&_t_=' . $token_like . '" title="Like">' . icon('like') . '</a>';
        $link_dislike = '<a href="' . $blog_data['full_url'] . '?_dislike_&_t_=' . $token_like . '" title="Dislike">' . icon('dislike') . '</a>';
    ...
    public function manager($app = array('index'))
    {
        ...
        $menu = get_apps(__SITE_PATH . '/modules/blog/apps/manager', 'blog/manager');

        ZenView::set_menu('application', $menu);
