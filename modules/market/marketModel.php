...
Class marketModel Extends ZenModel
{

    public $market_data = array();
    public $market_insert_id = 0;
    ...
    /**
     * check a market is exists
     * @param int $id
     * @return boolean
     */
    public function market_exists($id = 0)
    {
        if ($id == 0) {
        ...
        $_sql = "SELECT * FROM " . tb() . "markets where $this->select_recycle_bin and `id` = '$id'";
        ...
        $this->market_data = $this->db->fetch_array($query);

        return true;
    }

    /**
     * check a market is existed
     *
     * @param string $name
     * @param bool $without
     * @return bool
     */
    public function market_name_exists($name = '', $without = false)
    {
        $name = $this->db->sqlQuote(h($name));

        $sql = "SELECT `id` FROM " . tb() . "markets where $this->select_recycle_bin and `name` = '$name'";
    ...
    /**
     * get data of a market by id
     * @param int $sid
     * @param string $what_gets
     * @return array
     */
    public function get_market_data($sid = null, $what_gets = '*')
    {
        ...
        $_sql = "SELECT $select_what FROM " . tb() . "markets where `id`='$sid'";
     ...
     */
    public function get_list_market($parent = 0, $types = NULL, $order = array('weight' => 'ASC', 'time' => 'DESC'), $limit = false)
    {
        ...
        $_sql_count = "SELECT `id` FROM " . tb() . "markets where $this->select_recycle_bin $select_parent $select_type order by $select_order";
        ...
        $_sql = "SELECT $this->what_gets FROM " . tb() . "markets where $this->select_recycle_bin $select_parent $select_type order by $select_order $select_limit";
    ...
    public function gets($what_gets, $where, $order = array(), $limit = false)
    {
        ...
        $_sql_total = "SELECT $select_what FROM " . tb() . "markets $we order by $select_order";
        ...
        if ($this->db->num_row($re) == 1) {
            ...
            $markets[] = $this->gdata($this->db->fetch_array($re));

        } else {

            while ($row = $this->db->fetch_array($re)) {

                $markets[] = $this->gdata($row);
            }
        }
        /**
         * set the new cache
         */
        ZenCaching::set($_sql, $markets, 600);
    ...
    public function get_tags($sid)
    {
        ...
        $_sql = "SELECT * FROM " . tb() . "tags where `sid`='$sid' and `type` = 'market' order by `id` DESC";
        /**
         * get cache
         */
        $cache = ZenCaching::get($_sql);
     ...
     * @return array
     */
    public function get_tags_market($sid)
    {
        $tags = array();
        ...
        if ($this->db->num_row($query)) {
            ...
            while ($row = $this->db->fetch_array($query)) {
                ...
                $row['full_url'] = _HOME . '/search-' . $row['url'];
    ...
    public function add_tags($tags = array(), $sid)
    {
        ...
        foreach ($new_tags as $tag) {
            ...
            $tag = trim($tag);
            $query = "SELECT `id` FROM " . tb() . "tags where `sid` = '$sid' and `tag` = '$tag' and `type` = 'market'";
            ...
            if (!$this->db->num_row($re)) {
                ...
                $this->db->query("INSERT INTO " . tb() . "tags SET `sid` = '$sid', `url` = '$url', `tag` = '$tag', `time` = '" . time() . "', `type` = 'market'");
            }
        }
         ...
         * clean cache
         */
        ZenCaching::clean('get_tags_market');
    ...
    function delete_all_tags($sid) {
        ...
        $this->db->query("DELETE FROM " . tb() . "tags where `sid`='$sid' and `type` = 'market'");
        ZenCaching::clean();
    }
    ...
    public function insert_comment($data)
    {
        ...
        $sql = $this->db->_sql_insert(tb() . "markets_comments", $data);
    ...
    public function get_comments($sid, $limit = false)
    {
        ...
        $sql_total = "SELECT * FROM " . tb() . "markets_comments where `sid` = '$sid'";
    ...
    function delete_all_comments($sid) {
        ...
        $this->db->query("DELETE FROM " . tb() . "markets_comments where `sid`='$sid'");
        ZenCaching::clean();
    }
    ...
    public function get_like($sid)
    {
        ...
        $query = $this->db->query("SELECT `id` FROM " . tb() . "likes where `toid`='$sid' and `type` = 'market'");
    ...
    public function get_dislike($sid)
    {
        ...
        $query = $this->db->query("SELECT `id` FROM " . tb() . "dislikes where `toid`='$sid' and `type` = 'market'");
    ...
    public function is_liked($sid)
    {
        ...
        if (isset($this->user['id']) && !empty($this->user['id'])) {
            ...
            $query = $this->db->query("SELECT `id` FROM " . tb() . "likes where `toid` = '$sid' and `fromid` = '$uid' and `type` = 'market'");

        } else {

            $ip = client_ip();

            $query = $this->db->query("SELECT `id` FROM " . tb() . "likes where `toid` = '$sid' and `ip` = '$ip' and `fromid` = '0' and `type` = 'market'");
        }
    ...
    public function is_disliked($sid)
    {
        if (isset($this->user['id']) && !empty($this->user['id'])) {
            ...
            $query = $this->db->query("SELECT `id` FROM " . tb() . "dislikes where `toid`='$sid' and `fromid`='$uid' and `type` = 'market'");

        } else {

            $ip = client_ip();

            $query = $this->db->query("SELECT `id` FROM " . tb() . "dislikes where `toid` = '$sid' and `ip` = '$ip' and `fromid` = '0' and `type` = 'market'");
        }
            ...
            if (empty($data['toid'])) {
                ...
                if ($this->is_disliked($data['toid'])) {
                    ...
                    if (!empty($data['fromid'])) {
                        ...
                        $this->db->query("DELETE FROM " . tb() . "dislikes where `fromid` = '" . $data['fromid'] . "' and `toid` = '" . $data['toid'] . "' and `type` = 'market'");
                    }

                    if (!empty($data['ip'])) {

                        $this->db->query("DELETE FROM " . tb() . "dislikes where `ip` = '" . $data['ip'] . "' and `toid` = '" . $data['toid'] . "' and `fromid` = '0' and `type` = 'market'");
                    }
            ...
            if (empty($data['toid'])) {
                ...
                if ($this->is_liked($data['toid'])) {
                    ...
                    if (!empty($data['fromid'])) {
                        ...
                        $this->db->query("DELETE FROM " . tb() . "likes where `fromid` = '" . $data['fromid'] . "' and `toid` = '" . $data['toid'] . "' and `type` = 'market'");
                    }

                    if (!empty($data['ip'])) {

                        $this->db->query("DELETE FROM " . tb() . "likes where `ip` = '" . $data['ip'] . "' and `toid` = '" . $data['toid'] . "' and `fromid` = '0' and `type` = 'market'");
                    }
    ...
    function delete_all_likes($sid) {
        ...
        $this->db->query("DELETE FROM " . tb() . "likes where `toid` = '$sid' and `type` = 'market'");
        ZenCaching::clean();
    }
    ...
    function delete_all_dislikes($sid) {
        ...
        $this->db->query("DELETE FROM " . tb() . "dislikes where `toid` = '$sid' and `type` = 'market'");
        ZenCaching::clean();
    }
    ...
    public function anti_flood()
    {
        ...
        $sql = "SELECT `time` FROM " . tb() . "markets where $this->select_recycle_bin and `uid`='$uid' order by `time` DESC limit 1";
     ...
     * @return boolean
     */
    public function insert_market($data = array())
    {
        ...
        $sql = $this->db->_sql_insert(tb() . "markets", $data_ins);
        ...
        $this->market_insert_id = $this->db->insert_id();

        $this->db->query("UPDATE " . tb() . "markets SET `weight`=`weight`+1 where `id`!='" . $this->market_insert_id . "' and `weight`>='" . $data_ins['weight'] . "'");
     ...
     * @return int
     */
    public function market_insert_id()
    {
        return $this->market_insert_id;
    }
     ...
     * @return boolean
     */
    public function update_market($data = array(), $where)
    {
        ...
        if (is_numeric($where)) {
            ...
            $ok = $this->db->query("UPDATE " . tb() . "markets SET $update where `id` = '$sid'");
            ...
            $ok = $this->db->query("UPDATE " . tb() . "markets SET $update $where ");
        }
    ...
    function update_view($sid)
    {
        ...
        $this->db->query("UPDATE " . tb() . "markets SET `view` = `view` + 1 where `id` = '$sid'");
    }

    public function list_parent($sid, $find_what = 'url')
    {

        $market = $this->get_market_data($sid, array('parent', 'url', 'title'));

        $parent = $market['parent'];

        $list_parent[] = $market[$find_what];


        while ($re_get_par = $this->db->query("SELECT `id`, `parent`, `$find_what` from " . tb() . "markets where $this->select_recycle_bin and `id`='$parent' order by `id` ASC")) {
    ...
    public function remove_to_recycle_bin($sid)
    {
        ...
        $query = $this->db->query("SELECT `parent`, `type`, `id` FROM " . tb() . "markets where `id` = '$sid'");
        ...
        $this->db->query("UPDATE " . tb() . "markets SET `recycle_bin` = '1' where `id` = '$sid'");

        if ($row['type'] == 'folder') {

            $query2 = $this->db->query("SELECT `parent`, `type`, `id` FROM " . tb() . "markets where `parent` = '" . $row['id'] . "'");
    ...
    {
        $_sql = "SELECT * FROM " . tb() . "markets_links where `sid`='$sid' order by `id` ASC";
    ...
    {
        $_sql = "SELECT * FROM " . tb() . "markets_links where `id`='$lid'";
        /**
         * load cache
         */
        $cache = ZenCaching::get($_sql);
    ...
    public function insert_link($data)
    {
        ...
        $_sql = $this->db->_sql_insert(tb() . "markets_links", $data);
    ...
    public function update_link($lid, $data)
    {
        ...
        $_sql = $this->db->_sql_update(tb() . "markets_links", $data, array("`id` = '$lid'"));
    ...
    public function delete_link($lid)
    {
        ...
        if ($this->db->query("DELETE FROM " . tb() . "markets_links where `id` = '$lid'")) {
    ...
    public function delete_all_links($sid) {
        ...
        $this->db->query("DELETE FROM " . tb() . "markets_links where `sid` = '$sid'");
        ZenCaching::clean();
    }
    //---------------------------------------------------
    ...
    public function get_files($sid)
    {
        ...
        $_sql = "SELECT * FROM " . tb() . "markets_files where `sid`='$sid' order by `id` ASC";
        /**
         * load cache
         */
        $cache = ZenCaching::get($_sql);
    ...
    {
        $_sql = "SELECT * FROM " . tb() . "markets_files where `id`='$fid'";
    ...
    public function insert_file($data)
    {
        ...
        $_sql = $this->db->_sql_insert(tb() . "markets_files", $data);
    ...
    public function update_file($fid, $data)
    {
        ...
        $_sql = $this->db->_sql_update(tb() . "markets_files", $data, array("`id` = '$fid'"));
    ...
    public function delete_file($fid)
    {
        ...
        if ($this->db->query("DELETE FROM " . tb() . "markets_files where `id` = '$fid'")) {
    ...
    public function get_images($sid, $type_get = '')
    {
        ...
        $_sql = "SELECT * FROM " . tb() . "markets_images where `sid`='$sid' and `type` = '$type_get' order by `id` ASC";
        /**
         * load cache
         */
        $cache = ZenCaching::get($_sql);
    ...
    {
        $_sql = "SELECT * FROM " . tb() . "markets_images where `id`='$img_id'";
    ...
    public function insert_image($data)
    {
        ...
        $_sql = $this->db->_sql_insert(tb() . "markets_images", $data);
    ...
    public function delete_image($imgid)
    {
        ...
        if ($this->db->query("DELETE FROM " . tb() . "markets_images where `id` = '$imgid'")) {
    ...
    function count_files($sid = 0)
    {
        ...
        $query = $this->db->query("SELECT `id` FROM " . tb() . "markets_files $where");
    ...
    function count_links($sid)
    {
        ...
        $query = $this->db->query("SELECT `id` FROM " . tb() . "markets_links $where");
    ...
    public function update_type_view($sid, $type_view, $type = '')
    {
         ...
         * clean cache
         */
        ZenCaching::clean('get_market_data');

        $market = $this->get_market_data($sid, 'type_view, type');
        ...
        if ($type == 'post') {
            ...
            if ($market['type'] == 'post') {
                ...
                $this->update_market($update, $sid);
                return true;

            } elseif ($market['type'] == 'folder') {

                $this->update_market($update, "`parent` = '$sid' and `type` = 'post'");
            ...
            if ($market['type'] == 'post') {
                ...
                $query = $this->db->query("SELECT `id` FROM " . tb() . "markets where `parent` = '$sid'  and `type` = 'folder'");
    ...
    public function market_path_count_parent($parent = 0)
    {
        $count = $this->db->num_row($this->db->query("SELECT 'id' FROM " . tb() . "markets where `id`='$parent'"));
        return $count;
    }

    public function market_path_get_parent($parent = 0)
    {
        $row = $this->db->fetch_array($this->db->query("SELECT `parent`,`name`, `title`, `url`,`id`,`type` FROM " . tb() . "markets where `id`='$parent'"));
    ...
    function get_tree_folder($start = 0) {
        ...
        $list = $this->get_list_market($start, 'folder');
    ...
    function get_all_folder_recyclebin($limit = false) {
        ...
        $_sql_total = "SELECT * FROM ".tb()."markets where `type` = 'folder' and `recycle_bin` = '1' order by `time` DESC ";
    ...
    function delete($sid) {
        ...
        $market = $this->get_market_data($sid);

        if (isset($market['full_path_icon'])) {

            @unlink($market['full_path_icon']);
         ...
         * delete complete
         */
        $this->db->query("DELETE FROM ".tb()."markets WHERE `id` = '$sid'");

        if ($market['type'] == 'folder') {

            $this->db->query("UPDATE ".tb()."markets SET `recycle_bin` = '1' WHERE `parent` = '$sid'");
    ...
    function restore($sid) {
        ...
        return $this->db->query("UPDATE ".tb()."markets SET `recycle_bin` = '0' where `id` = '$sid'");
    }
