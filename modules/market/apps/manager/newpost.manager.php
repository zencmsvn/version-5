...
$tree[] = url(_HOME . '/market/manager/newpost', $page_title);
...
if (empty($cid)) {
        ...
        if (isset($_POST['uri'])) {
            ...
            if (!empty($cid)) {
                ...
                if ($model->market_exists($cid) == false) {
                    ...
                    redirect(_HOME . '/market/manager/newpost/' . $cid . '/step2');
                }
            } else {
    ...
    $obj->view->data = $data;
    $obj->view->show('market/manager/newpost/step1');
    return;
}
...
if ($model->market_exists($cid) == false) {
    ...
    redirect(_HOME . '/market/manager/newpost');
}
...
switch ($step) {
    ...
    case 'step2':
        ...
        if (!empty($_COOKIE['cookie_type_data']) && $type_data != 'unset') {
            ...
            redirect(_HOME . '/market/manager/newpost/' . $cid . '/step3/' . $_COOKIE['cookie_type_data']);
        }
        ...
        if (isset($_POST['sub_type_data'])) {
            ...
            if (preg_match('/^html$/is', $_POST['sub_type_data']) or preg_match('/^bbcode$/is', $_POST['sub_type_data'])) {
                ...
                redirect(_HOME . '/market/manager/newpost/' . $cid . '/step3/' . $type_data);
            }
        }

        $data['page_title'] = 'Bước 2';
        $obj->view->data = $data;
        $obj->view->show('market/manager/newpost/step2');
    ...
    case 'step3':
        ...
        if (isset($_POST['sub_newpost'])) {
            ...
            if ($InsertData['name'] == false || !strlen($InsertData['content'])) {
                ...
                $obj->view->data = $data;
                $obj->view->show('market/manager/newpost/newpost');
                return;
            }

            /**
             * check if market name is exists
             */
            if ($model->market_name_exists($InsertData['name'])) {
             ...
             */
            $InsertData = $obj->hook->loader('market_data_before_to_database', $InsertData);

            /**
             * insert market
             */
            $ins_ok = $model->insert_market($InsertData);
             ...
             * get insert id
             */
            $sid = $model->market_insert_id();

            /**
             * if auto get image from market content
             */
            if (isset($_POST['auto_get_img'])) {
                ...
                $InsertData['content'] = strtr($InsertData['content'], $replace);

                $model->update_market($updateData, $sid);
    ...
    default :
        ...
        redirect(_HOME . '/market/manager/newpost/' . $cid . '/step2/' . $type_data);
