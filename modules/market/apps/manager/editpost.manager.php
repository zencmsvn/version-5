...
$tree[] = url(_HOME . '/market/manager/editpost', $page_title);
...
if (empty($ActID) or $model->market_exists($ActID) == false) {
        ...
        if (isset($_POST['uri'])) {
            ...
            if (!empty($cid)) {
                ...
                if ($model->market_exists($cid) == false) {
                    ...
                    $cmarket = $model->get_market_data($cid, 'type_data');

                    redirect(_HOME . '/market/manager/editpost/' . $cid . '/step3/' . $cmarket['type_data']);
                }
            } else {
    ...
    $obj->view->data = $data;
    $obj->view->show('market/manager/editpost/step1');
    return;
}
 ...
 * get market data
 */
$market = $model->get_market_data($ActID);

/**
 * make sure market type is post
 */
if ($market['type'] != 'post') {
...
$market['check_url_only_me'] = '';
$market['check_url_with_parent'] = '';
$market['check_url_with_full_parent'] = '';

switch ($market['type_url']) {

    case 'only_me':

        $market['check_url_only_me'] = 'checked';
        break;

    case 'with_parent':

        $market['check_url_with_parent'] = 'checked';
        break;

    case 'with_full_parent':

        $market['check_url_with_full_parent'] = 'checked';
        break;
}


$market['check_title_only_me'] = '';
$market['check_title_with_parent'] = '';
$market['check_title_with_full_parent'] = '';
$market['check_title_custom'] = '';

switch ($market['type_title']) {
    case 'only_me':
        $market['check_title_only_me'] = 'checked';
        break;
    case 'with_parent':
        $market['check_title_with_parent'] = 'checked';
        break;
    case 'with_full_parent':
        $market['check_title_with_full_parent'] = 'checked';
        break;
    case 'custom':
        $market['check_title_custom'] = 'checked';
        break;
}
...
if ($market['check_title_custom']) {

    $market['custom_title'] = $market['title'];
}

$market['content'] = h($market['content']);


if (!empty($type_data)) {

    $market['type_data'] = $type_data;

} else {

    $type_data = $market['type_data'];
}
...
switch ($step) {
    ...
    case 'step2':
        ...
        if (!empty($_COOKIE['cookie_type_data']) && $type_data != 'unset') {
            ...
            redirect(_HOME . '/market/manager/editpost/' . $ActID . '/step3/' . $_COOKIE['cookie_type_data']);
        }
        ...
        if (isset($_POST['sub_type_data'])) {
            ...
            if (preg_match('/^html$/is', $_POST['sub_type_data']) or preg_match('/^bbcode$/is', $_POST['sub_type_data'])) {
                ...
                redirect(_HOME . '/market/manager/editpost/' . $ActID . '/step3/' . $type_data);
            }
        }

        $data['page_title'] = 'Bước 2';
        $obj->view->data = $data;
        $obj->view->show('market/manager/editpost/step2');
        return;
        break;
    ...
    case 'step3':
        ...
        if (isset($_POST['sub_move'])) {
            ...
            if (empty($data_moveto)) {
                ...
                if (!$model->update_market($update, $market['id'])) {

                    $data['notices'][] = 'Lỗi dữ liệu';

                } else {

                    redirect($market['full_url']);
                }
            }
        }
        ...
        if (isset($_POST['sub_editpost'])) {
            ...
            $cat = $model->get_market_data($market['parent']);

            $updateData['parent'] = $market['parent'];
            ...
            if ($updateData['name'] == false || !strlen($updateData['content'])) {
                ...
                $obj->view->data = $data;
                $obj->view->show('market/manager/editpost/editpost');
                return;
            }


            if ($model->market_name_exists($updateData['name'], $ActID)) {
            ...
            /**
             * market_data_before_to_database hook *
             */
            $updateData = $obj->hook->loader('market_data_before_to_database', $updateData);

            /**
             * update data
             */
            $update_ok = $model->update_market($updateData, $ActID);

            if ($update_ok == false) {

                $data['errors'][] = 'Không thể ghi dữ liệu. Vui lòng thử lại';
                $obj->view->data = $data;
                $obj->view->show('market/manager/newpost/editpost');
                return;
            }
            ...
            if (isset($_POST['auto_get_img'])) {
                ...
                $updateData['content'] = strtr($updateData['content'], $replace);

                $model->update_market($updateData, $ActID);
            }
    ...
    default :
        ...
        redirect(_HOME . '/market/manager/editpost/' . $ActID . '/step2/' . $type_data);
