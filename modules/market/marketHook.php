...
class marketHook extends ZenHook
{
    ...
    function lists_in_folder($data)
    {
        ...
        $data['posts'] = $model->get_list_market($data['sid'], 'post', array('weight' => 'ASC', 'time' => 'DESC'), $sql_limit);
        ...
        $data['folders'] = $model->get_list_market($data['sid'], 'folder', array('weight' => 'ASC', 'time' => 'DESC'), $sql_limit);

        $total = $model->total_result;
        $p->setTotal($total);

        hook_data(_PUBLIC, 'market_folder_after_list_folder', $p->navi_page('?fp={fg}'));

        $data['same_cats'] = $model->get_list_market($data['parent'], 'folder', array('time' => 'DESC'), 5);
        ...
        $data['rand_posts'] = $model->get_list_market(null, 'post', array('RAND()' => ''), $limit_rand);
    ...
    function lists_in_post($data)
    {
        ...
        $data['other_cats'] = $model->get_list_market(null, 'folder', array('time' => 'DESC'), 5);

        $data['same_posts'] = $model->get_list_market($data['parent'], 'post', array('time' => 'DESC'), 5);
    ...
    function lists_in_index($data)
    {
        ...
        $data['new_posts'] = $model->get_list_market(null, 'post', array('time' => 'DESC'), 10);
        $data['hot_posts'] = $model->get_list_market(null, 'post', array('view' => 'DESC'), 5);
        $data['rand_posts'] = $model->get_list_market(null, 'post', array('RAND()' => ''));
        return $data;
    }
    ...
    function post_manager_bar($sid) {
        ...
        $arr[_HOME . '/market/manager/editpost/' . $sid . '/step3'] = 'Sửa';
        $arr[_HOME . '/market/manager/links/' . $sid . '/step2'] = 'Links';
        $arr[_HOME . '/market/manager/files/' . $sid . '/step2'] = 'Files';
        $arr[_HOME . '/market/manager/images/' . $sid . '/step2'] = 'Hình ảnh';
        $arr[_HOME . '/market/manager/delete/' . $sid . '/step2'] = 'Xóa';
        $arr[_HOME . '/market/manager'] = 'Đến trang quản lí';
    ...
    function folder_manager_bar($sid) {
        ...
        $arr[_HOME . '/market/manager/newpost/'.$sid.'/step2'] = 'Viết bài';

        $arr[_HOME . '/market/manager/cpanel/'.$sid] = 'Đến trang quản lí và cài đặt';
    ...
    function upload_file($sid) {
         ...
         * get market model
         */
        $model = $registry->model->get('market');

        $registry->hook->get('market');
        ...
        for ($i = 1; $i <= $num_up; $i++) {
            ...
            if (empty($check_what)) {
                ...
                if (isset($_POST['name' . $i]) && strlen($_POST['name' . $i])) {
                    ...
                    $upload->set_file_name($market['name'] . ' ' . $_POST['name' . $i]);
                }
    ...
    function upload_image($sid) {
        ...
        $upload->set_file_name($market['url']);
    ...
    function recycleBin_manager_bar($sid) {
        ...
        $act[] = url(_HOME . '/market/manager/recycleBin?remarket='.$sid, 'Khôi phục', cfm('Bạn chắc chắn khôi bài này?'));
        $act[] = url(_HOME . '/market/manager/recycleBin?move='.$sid, 'Di chuyển');
        $act[] = url(_HOME . '/market/manager/recycleBin?delete='.$sid, 'Xóa', cfm('Bạn chắc chắn xóa bài này?'));
