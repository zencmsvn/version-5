...
class RobotsDetect{
    ...
    public function set_user_agent($user_agent){
        ...
        }elseif(preg_match('/rambler/i', $user_agent)){
            $this->rambler_bots();
    ...
    private function rambler_bots(){

        $this->robot_name = 'Rambler';
        if(preg_match('/StackRambler\/2.0 \(MSIE incompatible\)/', $this->user_agent)){
            $this->robot_type = 'Rambler Bot';

        }elseif(preg_match('/StackRambler\/2.0/', $this->user_agent)){
            $this->robot_type = 'Rambler Bot';
