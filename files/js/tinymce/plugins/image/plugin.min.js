tinymce.PluginManager.add("image", function (e) {
    ...
    function i(n) {
        ...
        u = f.getAttrib(h, "width"), m = f.getAttrib(h, "height"), "IMG" != h.nodeName || h.getAttribute("data-mce-object") ? h = null : d = {src: f.getAttrib(h, "src"), alt: f.getAttrib(h, "alt"), width: u, height: m}, n && (g = {name: "target", type: "listbox", label: "Danh sách ảnh", values: i(), onselect: function (e) {
            var t = c.find("#alt");
            (!t.value() || e.lastControl && t.value() == e.lastControl.text()) && t.value(e.control.text()), c.find("#src").value(e.control.value())
        }});
        var p = [
            {name: "src", type: "filepicker", filetype: "image", label: "Url ảnh", autofocus: !0, onchange: l},
            g,
            {name: "alt", type: "textbox", label: "Mô tả ảnh (alt)"},
            {type: "container", label: "Kích thước", layout: "flex", direction: "row", align: "center", spacing: 5, items: [
                {name: "width", type: "textbox", maxLength: 3, size: 3, onchange: a},
                {type: "label", text: "x"},
                {name: "height", type: "textbox", maxLength: 3, size: 3, onchange: a},
                {name: "constrain", type: "checkbox", checked: !0, text: "Giữ nguyên tỉ lệ"}
            ]}
        ];
        e.settings.image_advtab ? (h && (d.hspace = o(h.style.marginLeft || h.style.marginRight), d.vspace = o(h.style.marginTop || h.style.marginBottom), d.border = o(h.style.borderWidth), d.style = e.dom.serializeStyle(e.dom.parseStyle(e.dom.getAttrib(h, "style")))), c = e.windowManager.open({title: "Hình ảnh", data: d, bodyType: "tabpanel", body: [
        ...
        ], onSubmit: r})) : c = e.windowManager.open({title: "Chỉnh sửa ảnh", data: d, body: p, onSubmit: r})
    }

    e.addButton("image", {icon: "image", tooltip: "Hình ảnh", onclick: n(i), stateSelector: "img:not([data-mce-object])"}), e.addMenuItem("image", {icon: "image", text: "Insert image", onclick: n(i), context: "insert", prependToContext: !0})
});