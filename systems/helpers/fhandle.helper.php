...
if (!function_exists('scan_modules')) {
    ...
    {
        static $function_scan_modules;
        if (!empty($function_scan_modules)) {
            return $function_scan_modules;
        }
    ...
    function scan_templates()
    {
        ...
        foreach ($temp as $k => $t) {
            ...
            if (!file_exists($t . '/config.php') || !file_exists($t . '/run.php') || !file_exists($t . '/' . $name . '.info') || !is_dir($t . '/' . __FOLDER_TPL_NAME)) {
                ...
                $list[$name]['url'] = $name;
                $list[$name]['key'] = $name;
                $list[$name]['url_edit'] = _HOME . '/admin/tools/fileManager?file=templates/' . $name;
                $list[$name]['url_uninstall'] = _HOME . '/admin/general/templates/uninstall/' . $name;
